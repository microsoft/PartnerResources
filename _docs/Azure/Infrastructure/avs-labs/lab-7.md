...
permalink: /azure/infrastructure/azure-vmware-solution/avs-labs/lab-7
tags: 
 - azure
 - infrastructure
 - AVS
...
1. Go to Azure Portal home page, click on **Create a resource** button.
    ...
    ![create a resource]({{ site.baseurl }}/assets/avs-labs/lab-7/create-a-resource.png)

2. Select (or search for) **Storage account**.  

    ![select storage account]({{ site.baseurl }}/assets/avs-labs/lab-7/select-storage-account.png)
...
3. Fill out the main fields at the **Basics** section to start creating a new
   ...
   ![storage account basics]({{ site.baseurl }}/assets/avs-labs/lab-7/storage-account-basics.png)
...
5. Move to **Networking** section, and select **Private endpoint**. Then click
   ...
   ![storage account networking]({{ site.baseurl }}/assets/avs-labs/lab-7/storage-acc-networking.png)
...
6. Fill out the fields in **Create private endpoint** pane.
   ...
   4. Under **Private DNS integration**, make sure to select **Yes** for
      **Integrate with private DNS zone**. Since it didn’t exist in this
      ...
      ![create private endpoint]({{ site.baseurl }}/assets/avs-labs/lab-7/create-private-endpoint.png)

7. Click Ok at the Create private endpoint pane, and you should see something
   similar to the following:

    ![private endpoints]({{ site.baseurl }}/assets/avs-labs/lab-7/private-endpoints.png)
...
9. Click on **Next** to move to **Review + Create** pane. After validation is
   ...
   ![review and create]({{ site.baseurl }}/assets/avs-labs/lab-7/review-and-create.png)

10. It will take few minutes, and the deployment will be done.

    ![deployment complete]({{ site.baseurl }}/assets/avs-labs/lab-7/deployment-complete.png)
...
12. Go to **Networking** section on the **Storage Account** blade. There you
    ...
    ![add firewall exception]({{ site.baseurl }}/assets/avs-labs/lab-7/add-firewall-exception.png)

13. Your IP should appear in the list there, like this:  

    ![ip exceptions]({{ site.baseurl }}/assets/avs-labs/lab-7/ip-exceptions.png)

14. Click on Save button to save changes:  

    ![firewall save]({{ site.baseurl }}/assets/avs-labs/lab-7/firewall-save.png)

15. Next, refresh your browser tab to make sure change has been reflected:  

    ![browser refresh]({{ site.baseurl }}/assets/avs-labs/lab-7/browser-refresh.png)
...
16. Create a new blob container by going to **Storage Explorer** section and
    **right click** on **BLOB CONTAINERS**, then click on **Create blob
    ...
    ![create blob container]({{ site.baseurl }}/assets/avs-labs/lab-7/create-blob-container.png)
...
17. Give the container a name, and make sure the **Public access level** is set
    ...
    ![container name]({{ site.baseurl }}/assets/avs-labs/lab-7/container-name.png)
...
18. Create a text file (.txt) name it as you want and type any text in it. Try
    ...
    ![create text file]({{ site.baseurl }}/assets/avs-labs/lab-7/create-text-file.png)

19. Upload the created text file (try.txt) to the blob container that you just
    created.

    ![upload file]({{ site.baseurl }}/assets/avs-labs/lab-7/upload-file.png)

20. Copy and store the URL of that text file blob as you will be using it in
    future steps. This how you do
    it:

    ![copy url]({{ site.baseurl }}/assets/avs-labs/lab-7/copy-url.png)
...
21. Let’s now remove the Firewall exception that you added earlier, as it is no
    ...
    ![remove ip exception]({{ site.baseurl }}/assets/avs-labs/lab-7/remove-ip-exception.png)
...
22. NOW, is testing time!
    ...
    2. You can also do curl to the blob URL and see that you can view the
       ...
       ![curl url]({{ site.baseurl }}/assets/avs-labs/lab-7/curl-url.png)
    ...
    3. This is what you get when you try to access the URL from your
       ...
       ![curl-response]({{ site.baseurl }}/assets/avs-labs/lab-7/curl-response.png)
